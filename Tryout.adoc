...
Open the project with Android Studio, give him some to finish the sync.
...
android:textSize:: is, you guess it, the font size for the text.

AppTheme:: is the style define globally for our app in the `AndroidManifest`. This means that our `textSize` of `24sp` will be applied everywhere by default. 
...
textAlignment="textEnd":: makes the text aligned to the right. 
textStyle="bold":: makes the text bold
inputType="number":: is the most important attribute. It tells android to present the user with a simplified keyboard to enter a keyboard.
...
Style the second input:: like in the last paragraph
...
The text is now in white so that the contrast between the two stay readable.
...
We use the attributes `layout_margin{Start,End,Top,Bottom}` and `padding{Start,End,Top,Bottom}` to get the spacing right. 
...
inline class:: This cool feature was introduced in Kotlin 1.3. It allows to wrap a simple type (like `Int`) in a class, without to actually pay the price of creating and destroying wrapper objects all the time.

Why we define a Dollar class at all?:: This is the most interesting point here is.

Handling money is surprisingly tricky. We can't just define `9 dollars` as an `Int`. What about `$1.42` then? 

But if we define them as a `Double`, we get a problem. How much is `$1` divided by 3 people? Yes `$0.33333333333....` Alas, computers don't handle those numbers correctly. I won't go into the details of floating-point arithmetic. Long story short: the right thing to do is to count the number of cents. `$1 divided by 3` is `33+33+34 cents`. 

There is a Java Money and Currency API that we could use. Because reinventing the wheel is bad, right? Well, not always. This API is a bit ugly and does 10 times more things that what we need. More importantly, we will learn lot of things in the process. Let's get started!
...
This class `Dollar` does not look very natural to use, doesn't it?
...
validateInput():: Well, the user can enter a bad input, like `cuviawof` of just enter nothing. Let's add this function to `MainActivity`.
...
${totalPrice()}:: 
We replace the static values in the strings by a call to the functions that we defined and tested before.
...
Download the materials:: If you haven't done it already, https://drive.google.com/drive/folders/1aE9V8GNKZiDUV4uA-x3Eoi2-eBGhLljc?usp=sharing[Download the material from Google Drive^]. Unzip the file `Jmfayard-Android-PreTryout-SP.zip` and open it in Android Studio. Practice will solidify what you just learned.
